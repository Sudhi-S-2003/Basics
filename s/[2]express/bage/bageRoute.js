const badgeColors = require("./badgeColors");
const fillColors = require("./fillColors");

/**
 * 🎨 Dynamic Badge Generator Route
 * Supports query params:
 *  - title: tooltip text
 *  - label: badge text
 *  - badgeColor: color name or hex (e.g., javascript or #f7df1e)
 *  - fillColor: text color name or hex (e.g., dark or #fff)
 *
 * Example:
 *   /badge?title=Sudhi&label=JavaScript&badgeColor=javascript&fillColor=dark
 */
const BageRoute = (req, res) => {
  let { title = "Badge", label = "Sudhi", badgeColor = "default", fillColor = "light" } = req.query;

  // 🧩 Safe color resolution (supports hex or name)
  const bgColor = badgeColor.startsWith("#") ? badgeColor : (badgeColors[badgeColor] || badgeColors.default);
  const textColor = fillColor.startsWith("#") ? fillColor : (fillColors[fillColor] || fillColors.default);

  // 🧠 Escape label for SVG text safety
  const escapeXML = (str = "") => str.replace(/[<>&'"]/g, c => ({
    "<": "&lt;", ">": "&gt;", "&": "&amp;", "'": "&apos;", '"': "&quot;"
  }[c]));

  const safeLabel = escapeXML(label);
  const safeTitle = escapeXML(title);

  // 🖼️ SVG Badge Template
  const badgeSVG = `
<svg xmlns="http://www.w3.org/2000/svg" width="140" height="28" role="img" aria-label="${safeLabel}">
  <title>${safeTitle}</title>
  <rect width="140" height="28" fill="${bgColor}" rx="5" ry="5"/>
  <g font-family="Verdana, Geneva, DejaVu Sans, sans-serif" text-anchor="middle" font-weight="bold">
    <image x="10" y="7" width="14" height="14"
      href="data:image/svg+xml;base64,"/>
    <text x="85" y="18" fill="${textColor}" font-size="14">${safeLabel}</text>
  </g>
</svg>`.trim();

  // ⚡ Headers
  res.setHeader("Content-Type", "image/svg+xml");
  res.setHeader("Cache-Control", "public, max-age=3600"); // 1 hour cache
  res.send(badgeSVG);
};

module.exports = BageRoute;
